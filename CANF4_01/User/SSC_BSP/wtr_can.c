...
CAN_HANDLER can1 = {                           //所用函数的集合
    .rx_MSG             = {0},                 //CAN报文结构体（但是为什么就一个0就完事了,,,,
 ...
 *
 */
uint8_t CAN1_Send_Msg(CAN_MSG *msg)     //DLC位储存消息长度
{                                       //CAN_MSG:数据帧,msg是接受到的消息
    // 选择数据的长度
    switch (msg->len) {
        case 0:
        ...
        default:
            return 0;
    }           //啊为什么不直接用hcan1_tx.DLC = msg->len  ?
    if (REMOTE_FRAME == msg->rtr)               //REMOTE_FRAME是自己定义的枚举型变量
        hcan1_tx.RTR = CAN_RTR_REMOTE; // 远程帧    hcan1_tx是CAN_TxHeaderTypeDef，hal库的帧结构体
    else
